...
for (s in 1:length(SEXOS)){
  ...
  for (i in 1:length(stats)){
    ...
    if (SEX != "ALL"){
      GPL1261 <- read.csv(paste0("RESULTS_",stat,"/",SEX,"/",SEX,"_GPL1261_",stat,".csv"), header = TRUE, row.names = 1)
      GPL6246 <- read.csv(paste0("RESULTS_",stat,"/",SEX,"/",SEX,"_GPL6246_",stat,".csv"), header = TRUE, row.names = 1)
      GPL6887 <- read.csv(paste0("RESULTS_",stat,"/",SEX,"/",SEX,"_GPL6887_",stat,".csv"), header = TRUE, row.names = 1)
      GPL6885 <- read.csv(paste0("RESULTS_",stat,"/",SEX,"/",SEX,"_GPL6885_",stat,".csv"), header = TRUE, row.names = 1)
      GPL16570 <- read.csv(paste0("RESULTS_",stat,"/",SEX,"/",SEX,"_GPL16570_",stat,".csv"), header = TRUE, row.names = 1)
    }
    ...
    print("Interrogando HKGs...")
    GPLs = c("GPL1261", "GPL6246", "GPL6887", "GPL6885", "GPL16570")
    HKGs = c("Hprt", "Gapdh","Ppia", "Ubc", "Rpl19", "Rn18s")
    for (i in 1:length(HKGs)){
      HKG = HKGs[i]
      if (HKG !="Ubc" && HKG != "Rn18s"){
      ...
      if (HKG == "Ubc"){
        for (j in 1:length(GPLs)){
          GPLID = GPLs[j]
          if (GPLID != "GPL6887"){ # GPL6887 no contiene el gen Ubc
            df = get(GPLID)  
            which(rownames(df) == HKG)
            df_aux = df[HKG,]
            if (j == 1){HKG_df = df_aux}
          ...
          Ubc = HKG_df
          write.csv(HKG_df,paste0("./RESULTS_",stat,"/",SEX,"/HKG/Ubc_",stat,".csv"), row.names = TRUE)
        }
      }
      if (HKG == "Rn18s"){
        for (j in 1:length(GPLs)){
          GPLID = GPLs[j]
          if (GPLID != "GPL6885" && GPLID != "GPL16570"){ # Estas 2 GPLs no contienen el gen Rn18s
            df = get(GPLID)  
            which(rownames(df) == HKG)
            df_aux = df[HKG,]
            if (j == 1){HKG_df = df_aux}
    ...
    print("Escribiendo resultados en el directorio HKG")
    write.csv(HKG_df,paste0("./RESULTS_",stat,"/",SEX,"/HKG/HKG_ranking",stat,".csv"), row.names = TRUE)
    
    ## Limpiamos el escritorio de trabajo:
    rm(all_HKG_df, all_HKG_stats, all_HKG_medCV, RNA18, RNA18S, RNA18S_stats,Ubc, Ubc_stats,df_aux, HKG_df, GPLID, HKG)
    ...
    print("Generando rankings globales por paltaforma")
    ...
    for (i in 1:length(GPLs)){
      ...
      print("Añadiendo estadisticos")
      df_aux = add_stats(d)
      print("Extrayendo estadisticos y ordenando")
    ...
    GPLs = c("GPL1261", "GPL6246", "GPL6887", "GPL6885", "GPL16570")
    nGenes = c(21496, 24213, 30886, 18120,26467)
